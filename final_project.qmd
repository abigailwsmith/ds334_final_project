...
pitching_cleaner <- pitching_cleaner |> mutate(pitch_type = case_when(pitch_type == "sl" ~ "slider", pitch_type == "ch" ~ "changeup", pitch_type == "cu" ~ "curveball", pitch_type == "si" ~ "sinker")) |> relocate(pitch_type)
```
...
```{r}
ggplot(pitchspeed, aes(x = pitch_speed)) + geom_histogram() + facet_wrap(~year)
```

```{r}
ggplot(pitchspin, aes(x = spin_rate)) + geom_histogram() + facet_wrap(~year)
```
...
```{r}
slice_speed <- pitchspeed |> arrange(desc(pa)) |> slice(1:20)
```

```{r}
slice_spin <- pitchspin |> arrange(desc(pa)) |> slice(1:20)
```

```{r}
slice_attendance <- attendance_tv |> arrange(desc(Attendance)) |> slice(1:30)
```

```{r}
speed_new <- pitchspeed |> mutate(pitch_type = str_remove(pitch_type, "_avg_speed")) |> relocate(pitch_type)

speed_pitch <- speed_new |> mutate(pitch_type = case_when(pitch_type == "sl" ~ "slider", pitch_type == "ch" ~ "changeup", pitch_type == "cu" ~ "curveball", pitch_type == "si" ~ "sinker")) |> relocate(pitch_type)

spin_new <- pitchspin |> mutate(pitch_type = str_remove(pitch_type, "_avg_spin")) |> relocate(pitch_type)

spin_pitch <- spin_new |> mutate(pitch_type = case_when(pitch_type == "sl" ~ "slider", pitch_type == "ch" ~ "changeup", pitch_type == "cu" ~ "curveball", pitch_type == "si" ~ "sinker")) |> relocate(pitch_type)
```
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectizeInput("speed_pitch_sel", label = "Choose a pitch type for pitch speed:",
                     choices = pitchspeed$pitch_type, selected = "sl_avg_speed",
                     multiple = TRUE),
      ...
      selectizeInput("spin_pitch_sel", label = "Choose a pitch type for spin rate:",
                     choices = pitchspin$pitch_type, selected = "sl_avg_spin",
                     multiple = TRUE),
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Static Data", plotOutput("attendance_line"), plotOutput("audience_bar"), plotOutput("speedhist"), plotOutput("spinhist")), tabPanel("Data Tables", dataTableOutput("speed_raw_data"), dataTableOutput("spin_raw_data"), dataTableOutput("attendance_raw_data"))
      )
    )
  )
...
```{r}
scatter <- ggplot(pitching_cleaner, aes(x= pitch_speed, y = spin_rate, colour = pitch_type, label = year)) + geom_point(alpha = 0.2) + theme_minimal() + labs(title = "Plotly scatterplot of spin_rate against pitch speed in which the year of each pitch is hown when hovering over")
  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Static Data", plotlyOutput("interscatter"), plotOutput("attendance_line"), plotOutput("audience_bar"))
      )
    )
  )
