...
```{r}
library(shiny)
year_choices <- pitching_cleaner |> distinct(year) |> pull(year)
x_var_choices <- names(pitching_cleaner)[c(11, 13)]
y_var_choices <- names(pitching_cleaner)[c(5:10)]
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      radioButtons("year_sel", label = "Choose a year:", choices = year_choices),
      radioButtons("x_var_sel", label = "Choose an x-variable:", choices = x_var_choices),
      selectInput("y_var_sel", label = "Choose a y-variable:", choices = y_var_choices)
    ),
...
output$scatterplot <- renderPlot({ggplot(pitching_reactive(), aes(x= .data[[input$x_var_sel]], y = .data[[input$y_var_sel]], colour = pitch_type)) + geom_point() + theme_minimal()})
