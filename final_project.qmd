...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      ...
      radioButtons("pitch_sel", label = "Choose pitchtype:", choices = pitch_choice)
    ),
    mainPanel(
      tabsetPanel(
...
server <- function(input, output, session) {
 pitching_reactive <- reactive({
   pitching1 <- pitching_cleaner |> filter(year == input$year_sel) |> filter(pitch_type == input$pitch_sel) 
})
output$scatterplot <- renderPlot({ggplot(pitching_reactive(), aes(x= .data[[input$x_var_sel]], y = .data[[input$y_var_sel]])) + geom_point() + theme_minimal() + labs(title = glue::glue("Scatterplot of ", input$x_var_sel, " against ",  input$y_var_sel, " in ", input$year_sel))})
