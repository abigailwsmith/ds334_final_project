...
```{r}
attendance_smaller <- attendancefull |> select(Tm, Attendance, `Attend/G`, Time, `Est. Payroll`)
```
...
```{r}
pitching_cleaner |> group_by(pitch_type, pitcher, pitch_speed, spin_rate) |> summarise()
```
